...
Include `slave_class.hpp`, construct `LibXR::EtherCAT::SlaveClass` with its
`Configuration`, and call `Initialize`, `Poll`, `Worker`, `Process`, or `Run` from
the application/driver. The application or board driver supplies the SOES headers and
sources, ESC callbacks, and the global `cb_get_inputs`/`cb_set_outputs` process-data
callbacks.
