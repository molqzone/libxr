 ...
 * - optional ErrorCode RvSwdSeqWriteBits(uint32_t, const uint8_t*)
 * - ErrorCode RvSwdReadBits(uint32_t, uint8_t*)
 * - ErrorCode RvSwdPrepareReadTurnaround()
 * - ErrorCode RvSwdStart()
 * - ErrorCode RvSwdStop()
 */
...
class RvSwdGeneralGPIO final : public RvSwd
{
 public:
  ...
  explicit RvSwdGeneralGPIO(BitPort& bit_port) : port_(bit_port) {}
                             ...
                             SwdTransferCapture& capture)
  {
    ...
    const bool wrap_frame = (flags & 0x80u) != 0u;
    if (wrap_frame)
    {
      const ErrorCode start_ec = port_.BeginRvSwdFrame();
      if (start_ec != ErrorCode::OK)
      {
        capture.last_ec = static_cast<int8_t>(start_ec);
        debug_snapshot_.last_ec = capture.last_ec;
        return start_ec;
      }
    }

    SwdProtocol::Request req = {};
    req.port = ((flags & 0x01u) != 0u) ? SwdProtocol::Port::AP : SwdProtocol::Port::DP;
    req.rnw = (flags & 0x02u) != 0u;
    req.addr2b = capture.addr2b;
    req.wdata = wdata;

    SwdProtocol::Response resp = {};
    ErrorCode ec = ErrorCode::OK;
    if (((flags & 0x10u) != 0u) && req.port == SwdProtocol::Port::AP && req.rnw)
    {
      SwdProtocol::Ack ack = SwdProtocol::Ack::PROTOCOL;
      uint32_t rdata = 0u;
      ec = port_.ApReadTxn(capture.addr2b, rdata, ack);
      capture.ack = static_cast<uint8_t>(ack);
      capture.parity_ok =
          (ec == ErrorCode::OK && ack == SwdProtocol::Ack::OK) ? static_cast<uint8_t>(1u)
                                                                : static_cast<uint8_t>(0u);
      capture.rdata = rdata;
    }
    else
    {
      const bool use_retry = ((flags & 0x10u) != 0u) || ((flags & 0x20u) != 0u);
      ec = use_retry ? port_.TransferWithRetry(req, resp) : port_.Transfer(req, resp);
      capture.ack = static_cast<uint8_t>(resp.ack);
      capture.parity_ok = resp.parity_ok ? static_cast<uint8_t>(1u) : static_cast<uint8_t>(0u);
      capture.rdata = resp.rdata;
    }

    if (wrap_frame)
    {
      const ErrorCode end_ec = port_.EndRvSwdFrame();
      if (ec == ErrorCode::OK)
      {
        ec = end_ec;
      }
                                ...
                                SwdRawCapture& capture)
  {
    ...
    const uint8_t req_byte =
        EncodeSwdRequestByte((flags & 0x01u) != 0u, (flags & 0x02u) != 0u, capture.addr2b);
    ...
    ErrorCode ec = port_.SeqWriteBits(8u, &req_byte);
    if (ec == ErrorCode::OK)
    {
      ec = port_.SeqReadBits(bit_count, capture.raw_bits.data());
    }
                          ...
                          SwdBatchCapture& capture)
  {
    ...
    capture.count = count;
    if (ops == nullptr || count == 0u || count > static_cast<uint8_t>(capture.results.size()))
    {
      capture.last_ec = static_cast<int8_t>(ErrorCode::ARG_ERR);
      return ErrorCode::ARG_ERR;
    }

    ErrorCode last_ec = ErrorCode::OK;
    for (uint8_t i = 0u; i < count; ++i)
    {
      SwdTransferCapture single = {};
      uint8_t flags = ops[i].flags;
      if ((batch_flags & 0x80u) != 0u)
      {
        flags = static_cast<uint8_t>(flags | 0x80u);
      }
      if ((batch_flags & 0x01u) != 0u && i == 0u)
      {
        flags = static_cast<uint8_t>(flags | 0x40u);
      }

      last_ec = DebugSwdTransfer(flags, ops[i].addr2b, ops[i].wdata, single);
      capture.results[i].flags = ops[i].flags;
      capture.results[i].addr2b = static_cast<uint8_t>(ops[i].addr2b & 0x03u);
      capture.results[i].last_ec = single.last_ec;
      capture.results[i].ack = single.ack;
      capture.results[i].parity_ok = single.parity_ok;
      capture.results[i].rdata = single.rdata;
      capture.last_ec = single.last_ec;
      if (last_ec != ErrorCode::OK)
      {
        break;
      }
    }
 ...
 private:
  ...
  static uint8_t EncodeSwdRequestByte(bool apndp, bool rnw, uint8_t addr2b)
  {
    ...
    const uint8_t parity =
        static_cast<uint8_t>((static_cast<uint8_t>(apndp) ^ static_cast<uint8_t>(rnw) ^ a2 ^
                              a3) &
                             0x01u);
    return static_cast<uint8_t>((1u << 0u) | (static_cast<uint8_t>(apndp) << 1u) |
                                (static_cast<uint8_t>(rnw) << 2u) | (a2 << 3u) |
                                (a3 << 4u) | (parity << 5u) | (1u << 7u));
  }
  ...
  static auto WriteBits(BitPort& port, uint32_t bits, const uint8_t* data_lsb_first, int)
      -> decltype(port.RvSwdSeqWriteBits(bits, data_lsb_first))
  {
    return port.RvSwdSeqWriteBits(bits, data_lsb_first);
  }

  static ErrorCode WriteBits(BitPort& port, uint32_t bits, const uint8_t* data_lsb_first, long)
  {
    return port.SeqWriteBits(bits, data_lsb_first);
  }

  static auto SendWakePulse(BitPort& port, int) -> decltype(port.RvSwdWakePulse())
  {
    return port.RvSwdWakePulse();
  }

  static ErrorCode SendWakePulse(BitPort&, long) { return ErrorCode::OK; }
  ...
  static ErrorCode EncodeAndWriteMsbFirst(BitPort& port, uint64_t payload, uint32_t bits)
  {
    ...
    return WriteBits(port, bits, packed.data(), 0);
  }

  static ErrorCode ReadMsbFirstU32RvSwd(BitPort& port, uint32_t bits, uint32_t& out)
  {
    if (bits == 0u || bits > 32u)
    {
      return ErrorCode::ARG_ERR;
    ...
    const ErrorCode ec = port.RvSwdReadBits(bits, packed.data());
    if (ec != ErrorCode::OK)
    {
      return ec;
    }
  ...
  static ErrorCode ReadMsbFirstU64RvSwd(BitPort& port, uint32_t bits, uint64_t& out)
  {
    if (bits == 0u || bits > 64u)
    {
      return ErrorCode::ARG_ERR;
  ...
  ErrorCode SendLegacyStopBit()
  {
    ...
    const ErrorCode ec = WriteBits(port_, 1u, &one_lsb, 0);
    if (ec != ErrorCode::OK)
    {
      return ec;
    }
  ...
  ErrorCode LegacyReadWordRaw(uint8_t wire_addr_with_rw, uint32_t& data)
  {
    ...
    ec = port_.RvSwdPrepareReadTurnaround();
    if (ec != ErrorCode::OK)
    {
      return ec;
    }
    ...
    ec = ReadMsbFirstU32(port_, kLegacyReadTurnaroundBits, turnaround);
    if (ec != ErrorCode::OK)
    {
      return ec;
    }

    ec = ReadMsbFirstU32(port_, kDataBits, data);
    if (ec != ErrorCode::OK)
    {
      return ec;
    }
                            ...
                            bool& payload_updated)
  {
    ...
    uint8_t cmd_lsb = cmd;
    ec = WriteBits(port_, 8u, &cmd_lsb, 0);
    if (ec != ErrorCode::OK)
    {
      (void)port_.RvSwdStop();
      return ec;
    }
    ...
    ec = port_.RvSwdReadBits(3u, ack_buf.data());
    if (ec != ErrorCode::OK)
    {
      (void)port_.RvSwdStop();
      return ec;
    ...
    if (bridge_ack == kBridgeAckOk && payload_mode != BridgePayloadMode::None)
    {
      ...
      if (read_path)
      {
        ...
        ec = port_.RvSwdReadBits(32u, read_bytes.data());
        if (ec != ErrorCode::OK)
        {
          (void)port_.RvSwdStop();
          return ec;
      ...
      {
        ec = WriteBits(port_, 32u, active_payload.data(), 0);
        if (ec != ErrorCode::OK)
        {
          (void)port_.RvSwdStop();
          return ec;
  ...
  ErrorCode WriteWordRaw(uint8_t dmi_addr, uint32_t data)
  {
    ...
    ErrorCode ec = port_.BeginRvSwdFrame();
    if (ec != ErrorCode::OK)
    {
      return ec;
    }

    ec = EncodeAndWriteMsbFirst(port_, frame, kWriteFrameBits);
    if (ec != ErrorCode::OK)
    {
      (void)port_.EndRvSwdFrame();
      return ec;
    }

    debug_snapshot_.stage = 0x11u;
    ec = port_.EndRvSwdFrame();
    if (ec == ErrorCode::OK)
    {
  ...
  ErrorCode ReadWordRaw(uint8_t dmi_addr, uint32_t& data)
  {
    ...
    ec = EncodeAndWriteMsbFirst(port_, header, kReadPrefixBits);
    if (ec != ErrorCode::OK)
    {
      (void)port_.EndRvSwdFrame();
      return ec;
    ...
    ec = ReadMsbFirstU32RvSwd(port_, kDataBits, data);
    if (ec != ErrorCode::OK)
    {
      (void)port_.EndRvSwdFrame();
      return ec;
    ...
    ec = port_.RvSwdReadBits(1u, parity_buf.data());
    if (ec != ErrorCode::OK)
    {
      (void)port_.EndRvSwdFrame();
      return ec;
    ...
    ec = EncodeAndWriteMsbFirst(port_, kRvSwdTail, kRvSwdHostTailBits);
    if (ec != ErrorCode::OK)
    {
      (void)port_.EndRvSwdFrame();
      return ec;
                           ...
                           uint32_t& target_data, uint8_t& target_status)
  {
    ...
    ec = EncodeAndWriteMsbFirst(port_, last_host_frame_, kDmiFrameBits);
    if (ec != ErrorCode::OK)
    {
      (void)port_.RvSwdStop();
      debug_snapshot_.last_ec = static_cast<int8_t>(ec);
    ...
    ec = ReadMsbFirstU64RvSwd(port_, kDmiFrameBits, raw_target_frame);
    if (ec != ErrorCode::OK)
    {
      (void)port_.RvSwdStop();
      debug_snapshot_.last_ec = static_cast<int8_t>(ec);
